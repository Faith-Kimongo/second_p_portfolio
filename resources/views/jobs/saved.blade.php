            ...
            <div class="grid grid-row-1 gap-4 sm:grid-cols-2">
              <div class="shadow-2xl m-2 rounded-2xl p-6">
                <ul role="list" class="divide-y divide-gray-200">
                    @forelse ($jobs as $job)
                    <a href=" {{route('job.show',$job->job->slug)}} " class="block hover:bg-gray-50">
                        <div class="px-4 py-4 sm:px-6">
                          <div class="flex items-center justify-between">
                            <p class="truncate text-sm font-medium text-pink-700"> {{$job->job->title}} </p>
                            <div class="ml-2 flex flex-shrink-0">
                                    <a href="{{route('job.apply',$job->job->slug)}}" type="button" class="inline-flex items-center rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium leading-4 text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-30">Apply<span class="sr-only">, Hobby</span></a>
                            </div>
                          </div>
                    ...
                    @empty
                        <li>
                          <x-alert :message="'No jobs saved yet'"></x-alert>
                        </li>
                    @endforelse
